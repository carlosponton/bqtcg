-- create_listing() escribe `listings.languages` (text[]) en vez de `language`.
--   payload.languages = ["es","en"]  -> esos idiomas
--   payload.languages = [] o ausente -> {"any"} = "cualquier idioma"
-- Conserva ciudad (20260909000002) y decks (20260908000002). Un archivo por
...
declare
  ...
  v_languages text[] := coalesce(
    (select array_agg(distinct x)
       from jsonb_array_elements_text(payload -> 'languages') as x
      where x in ('es', 'en', 'jp', 'pt', 'fr', 'de', 'it', 'other')),
    '{}'::text[]
  );
  v_any_lang constant text[] := array['any'];
...
begin
  ...
  if v_city is null then
    raise exception 'Elige tu ciudad.' using errcode = '23514';
  end if;
